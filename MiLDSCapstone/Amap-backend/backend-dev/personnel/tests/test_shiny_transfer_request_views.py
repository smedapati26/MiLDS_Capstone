...
from personnel.models import UserRole, SoldierTransferRequest
from personnel.model_utils import UserRoleAccessLevel, Echelon

from utils.tests import create_test_soldier, create_test_unit, create_test_transfer_request, create_user_role_in_all
...
class SoldierTransferRequestTests(TestCase):
    ...
    def setUp(self) -> None:
        # Create Units
        self.current_owning_bn = create_test_unit(uic="OWNING_BN", echelon=Echelon.BATTALION)
        self.current_unit = create_test_unit(
            uic="OWNING",
            short_name="A CO, 1-100 TEST",
            display_name="Alpha Company, 1st Battalion, 100th Test Aviation Regiment",
            echelon=Echelon.COMPANY,
            parent_uic=self.current_owning_bn,
        )
        self.requesting_unit_1 = create_test_unit(
            uic="REQUEST1",
            short_name="B CO, 2-100 TEST",
            display_name="Bravo Company, 2nd Battalion, 100th Test Aviation Regiment",
            echelon=Echelon.COMPANY,
        )
        self.requesting_unit_2 = create_test_unit(
            uic="REQUEST2",
    ...
    def test_get_transfer_requests_with_invalid_user(self):
        ...
        response = self.client.get(
            url,
            headers={"X_ON_BEHALF_OF": "NOT" + self.test_owning_bn_admin.user_id},
    ...
    @tag("validation")
    def test_valid_get_transfer_request_from_admin_user(self):
        """
        ...
        response = self.client.get(
            url,
            headers={"X_ON_BEHALF_OF": self.test_amap_admin.user_id},
            content_type=self.json_content,
        )
    ...
    def test_valid_get_transfer_request_invalid_get_type(self):
        ...
        response = self.client.get(
            url,
            headers={"X_ON_BEHALF_OF": self.test_owning_bn_admin.user_id},
            content_type=self.json_content,
        )
        self.assertEqual(response.status_code, HTTP_BAD_SERVER_STATUS_CODE)
    ...
    @tag("validation")
    def test_valid_get_transfer_requests_for_non_admin_user(self):
        """
        Checks that a valid get request for soldier transfers pending their adjudication
        from a non amap admin user only returns user requests for soldier in the units that the
        ...
        user_admin_roles = UserRole.objects.filter(
            user_id=self.test_owning_bn_admin.user_id, access_level=UserRoleAccessLevel.ADMIN
        )
        users_units = []
    ...
    @tag("validation")
    def test_valid_get_transfer_requests_from_non_admin_user(self):
        """
        Checks that a valid get request for open soldier transfer requests from a non amap
        admin user only returns user requests for soldier in the units that the requesting
        soldier is a unit admin/manager for
        """
    ...
    def test_post_valid_transfer_request(self):
        ...
        soldier_request = SoldierTransferRequest.objects.filter(
            requester=self.test_owning_bn_admin, gaining_unit=self.requesting_unit_1, soldier=self.test_second_soldier
        )
