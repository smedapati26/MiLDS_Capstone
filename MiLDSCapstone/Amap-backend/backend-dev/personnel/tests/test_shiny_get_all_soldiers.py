...
class GetAllSoldiersTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.test_unit_2 = create_test_unit(uic="TEST01AA")
        # Create Maintainer
    ...
    @tag("validation")
    def test_get_no_soldiers_if_not_admin(self):
        """
    ...
    @tag("validation")
    def test_get_all_soldiers_in_admin_units(self):
        """
        Checks that all related soldiers are retrieved
        """
        role = UserRole.objects.create(
            user_id=self.test_maintainer, unit=self.test_unit, access_level=UserRoleAccessLevel.ADMIN
        )
        url = reverse(self.get_soldier_url)
        headers = {"X-On-Behalf-Of": "1234567890"}
        response = self.client.get(url, headers=headers)
