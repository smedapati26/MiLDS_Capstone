...
from utils.tests import create_test_soldier, create_test_unit, create_test_user_request, create_user_role_in_all
...
class UserRequestTests(TestCase):
    ...
    def setUp(self) -> None:
        # Create Units
        self.test_bn = create_test_unit()
        self.test_co = create_test_unit(
            uic="TEST000A0", echelon=Echelon.COMPANY, short_name="A CO, 1-100 TEST", parent_uic=self.test_bn
        )
        self.test_second_co = create_test_unit(uic="TEST000B0", echelon=Echelon.COMPANY, short_name="A CO, 2-100 TEST")
        # Create Soldiers
