...
from django.http import HttpRequest, HttpResponseBadRequest, JsonResponse, HttpResponseNotFound, HttpResponseServerError
...
def equipment_transfer(request: HttpRequest):
    ...
    for object in objects_to_transfer:
        ...
        # Units that will have Aircraft removed.
        if permanent_update:
        ...
        if user_has_permissions_to(user, originating_unit, UserRoleAccessLevel.ADMIN) and user_has_permissions_to(
            ...
            if permanent_update:
                if object_type == TransferObjectTypes.AIR:
                    object.uic.remove(*unit_uics_that_will_have_object_deleted)
                else:
                    object.tracked_by_unit.remove(*unit_uics_that_will_have_object_deleted)

            # Unit Aircraft to be created if not already existing.
            if object_type == TransferObjectTypes.AIR:
                object.uic.add(*unit_uics_that_will_have_object_added)
            else:
                object.tracked_by_unit.add(*unit_uics_that_will_have_object_added)
