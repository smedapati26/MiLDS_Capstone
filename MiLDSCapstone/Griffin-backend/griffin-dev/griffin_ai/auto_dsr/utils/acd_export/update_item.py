...
def update_item(record: pd.Series, export_time: datetime, inspections_df: pd.DataFrame, phase_df: pd.DataFrame):
    """
    ...
    if "Q" in record["DailyStatus-End Item Model"]:
        update_uas(record, export_time)
    else:
        update_aircraft(record, export_time, inspections_df, phase_df)
