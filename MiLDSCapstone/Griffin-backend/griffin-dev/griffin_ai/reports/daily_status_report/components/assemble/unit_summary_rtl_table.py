...
def assemble_rtl_summary_table(rtl_df: pd.DataFrame) -> Table:
    """
    Assembles a formatted RTL summary table given an appropriately made DataFrame

    @param rtl_df (pandas.DataFrame) the RTL Summary DataFrame
    @returns (Table) the table object fully styled and ready to be drawn
    """
    location_names = rtl_df.index.get_level_values(0)
    longest_location = max(len(str(value)) for value in location_names)
