...
@tag("aircraft", "delete", "modification_category")
class ModificationCategoryDeleteTestCase(TestCase):
    def setUp(self):
    ...
    def test_get_with_non_existing_modification(self):
        response = self.client.delete(
            reverse(
                ...
                kwargs={"name": "DOESNOTEXIST", "value": self.modification_category.value},
            ),
        )
    ...
    def test_get_with_non_existing_modification_category(self):
        response = self.client.delete(
            reverse("delete_modification_category", kwargs={"name": self.modification.name, "value": "DOESNOTEXIST"}),
        )
    ...
    def test_get_with_valid_modification_category(self):
        ...
        self.assertEqual(response.content.decode("utf-8"), "Modification Category successfully deleted.")
