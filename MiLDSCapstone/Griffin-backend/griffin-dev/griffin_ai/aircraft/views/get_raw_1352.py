...
def get_raw_1352(request: HttpRequest, uic: str, start_date: date = None, end_date: date = None):
    """
    A view to the raw_1352 table in the database returning all 1352 records for
    ...
    data = Raw_DA_1352.objects.filter(reporting_uic__in=uics, reporting_month__range=(start_date, end_date)).values(
        *readiness_columns
    )
