...
def update_aircraft(request: HttpRequest, aircraft_serial: str):
    ...
    # Setting initial status for the response and potentail error strings.
    ...
    common_fields = ["rtl", "total_airframe_hours", "flight_hours", 
        "should_sync"]
    ...
    if "location_id" in data.keys() and data["location_id"] != {}:
        try:
    ...
    if "hours_to_phase" in data.keys() and data["hours_to_phase"] != {}:
        phase_defaults = {
    ...
    if "remarks" in data.keys() and data["remarks"] != {}:
