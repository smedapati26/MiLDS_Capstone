...
import { useUavTableFilter } from '@features/daily-status-report/components/EquipmentDetails/UAS/UAV/useUAVTableFilter';
...
describe('useUavTableFilter', () => {
  it('returns empty array when tableData is undefined', () => {
    const props = {
      tableData: undefined,
      searchQuery: '',
      filters: uavFilterDefaultValues,
      columns: mockColumns,
    };

    const { result } = renderHook(() => useUavTableFilter(props));
  ...
  it('filters data based on status filter', () => {
    const filters: UAVFilterSchemaType = {
      ...uavFilterDefaultValues,
      status: OperationalReadinessStatusEnum.FMC,
    };
