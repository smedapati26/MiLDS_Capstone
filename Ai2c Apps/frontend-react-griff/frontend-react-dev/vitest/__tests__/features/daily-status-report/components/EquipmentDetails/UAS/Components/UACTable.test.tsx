...
import { UACTable } from '@features/daily-status-report/components/EquipmentDetails/UAS/Components/UACTable';
...
vi.mock('./useUACTableFilter', () => ({
  useUacTableFilter: vi.fn(() => []),
}));
...
describe('UACTable', () => {
  it('renders without crashing', () => {
    vi.mocked(useGetUACQuery).mockReturnValue({ data: undefined, isLoading: false, refetch: vi.fn() });

    render(
      <ProviderWrapper>
        <UACTable uic="test-uic" />
      </ProviderWrapper>,
    );
  ...
  it('displays loading state', () => {
    vi.mocked(useGetUACQuery).mockReturnValue({ data: undefined, isLoading: true, refetch: vi.fn() });
  ...
  it('renders table with data', () => {
    ...
    vi.mocked(useGetUACQuery).mockReturnValue({ data: mockData, isLoading: false, refetch: vi.fn() });
  ...
  it('passes uic prop correctly', () => {
    ...
    expect(useGetUACQuery).toHaveBeenCalledWith({ uic: 'test-uic' }, { skip: false });
  });
  ...
  it('skips query when uic is undefined', () => {
    ...
    render(
      <ProviderWrapper>
        <UACTable uic={undefined} />
      </ProviderWrapper>,
    );

    expect(useGetUACQuery).toHaveBeenCalledWith({ uic: undefined }, { skip: true });
  });
});
