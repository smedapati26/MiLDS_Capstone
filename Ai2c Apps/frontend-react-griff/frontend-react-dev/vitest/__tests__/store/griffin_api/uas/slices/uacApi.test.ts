...
import { uacApi } from '@store/griffin_api/uas/slices/uacApi';
...
const createTestStore = () => {
  return configureStore({
    ...
    middleware: (getDefaultMiddleware) => getDefaultMiddleware().concat(uacApi.middleware),
  });
};
...
describe('uacApi', () => {
  describe('API Slice Configuration', () => {
    it('should have correct reducer path', () => {
      expect(uacApi.reducerPath).toBe('uacApi');
    });

    it('should have correct base query configuration', () => {
      expect(uacApi.endpoints).toBeDefined();
      expect(Object.keys(uacApi.endpoints)).toHaveLength(2);
    });

    it('should export all expected hooks', () => {
      expect(uacApi.useGetUACQuery).toBeDefined();
      expect(uacApi.useEditUacEquipmentMutation).toBeDefined();
    });

    it('should properly integrate with Redux store', () => {
      const state = createTestStore().getState();
      expect(state).toHaveProperty(uacApi.reducerPath);
    });

    it('should have correct keepUnusedDataFor configuration', () => {
      expect(uacApi).toHaveProperty('reducerPath');
    });

    it('should have getUAC endpoint defined', () => {
      expect(uacApi.endpoints.getUAC).toBeDefined();
    });

    it('should have editUacEquipment endpoint defined', () => {
      expect(uacApi.endpoints.editUacEquipment).toBeDefined();
    });
  });

  describe('useGetUACQuery', () => {
    ...
    it('should successfully fetch UAC by UIC', async () => {
      ...
      const { result } = renderHook(() => uacApi.useGetUACQuery({ uic: 'TEST_UIC' }), {
        wrapper,
      });
    ...
    it('should pass UIC as query parameter', async () => {
      ...
      const { result } = renderHook(() => uacApi.useGetUACQuery({ uic: 'CUSTOM_UIC' }), {
        wrapper,
      });
    ...
    it('should cache results correctly', async () => {
      ...
      const { result: result2 } = renderHook(() => uacApi.useGetUACQuery({ uic: 'TEST_UIC' }), {
        wrapper,
      });
    ...
    it('should not cache results for different UICs', async () => {
      ...
      const { result: result1 } = renderHook(() => uacApi.useGetUACQuery({ uic: 'TEST_UIC_1' }), {
        wrapper,
      });
      await waitFor(() => {
      ...
      const { result: result2 } = renderHook(() => uacApi.useGetUACQuery({ uic: 'TEST_UIC_2' }), {
        wrapper,
      });
    ...
    it('should handle server errors', async () => {
      ...
      const { result } = renderHook(() => uacApi.useGetUACQuery({ uic: 'server-error' }), {
        wrapper,
      });
    ...
    // Remove or modify this test - your mock returns data, not empty array
  ...
  describe('useEditUacEquipmentMutation', () => {
    let store: ReturnType<typeof createTestStore>;
    ...
    it('should successfully edit UAC equipment', async () => {
      const wrapper = createWrapper(store);

      const { result } = renderHook(() => uacApi.useEditUacEquipmentMutation(), {
        wrapper,
      });
  ...
  describe('Endpoint Integration', () => {
    ...
    it('should have both endpoints available in the store', () => {
      ...
      expect(state[uacApi.reducerPath]).toBeDefined();
    });

    it('should handle concurrent queries and mutations', async () => {
      const wrapper = createWrapper(store);

      const { result: queryResult } = renderHook(() => uacApi.useGetUACQuery({ uic: 'TEST_UIC' }), {
        wrapper,
      });

      const { result: mutationResult } = renderHook(() => uacApi.useEditUacEquipmentMutation(), {
        wrapper,
      });
