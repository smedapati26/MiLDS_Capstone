...
import { UAVFilterSchemaType } from './schema';

interface Props {
  tableData: IUAS[] | undefined;
  searchQuery: string;
  filters: UAVFilterSchemaType;
 ...
 */
export const useUavTableFilter = ({ tableData, searchQuery, filters, columns }: Props): IUAS[] => {
  return useMemo(() => {
    ...
    return (
      tableData
        ...
        .filter((row) => {
          ...
          // Filter by hours flown (only if checkbox is checked)
          const matchesHoursFlown =
            !filters.isHoursFlownChecked || // If not checked, include all
            (Number(row.flightHours) >= filters.hoursFlown[0] && Number(row.flightHours) <= filters.hoursFlown[1]);

          // Filter by hours to airframe (only if checkbox is checked)
          const matchesHoursToAirframe =
            !filters.isTotalAirframeHoursChecked || // If not checked, include all
            (Number(row.totalAirframeHours) >= filters.totalAirframeHours[0] &&
              Number(row.totalAirframeHours) <= filters.totalAirframeHours[1]);
