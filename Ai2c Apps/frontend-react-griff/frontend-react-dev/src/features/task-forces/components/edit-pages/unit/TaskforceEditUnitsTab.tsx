...
import { getUnitFormData } from '@features/task-forces/utils/getFormData';
...
import { editUnitsSchema, EditUnitsSchemaType } from './schema';
import { TaskforceEditUnitsForm } from './TaskforceEditUnitsForm';
...
export const TaskforceEditUnitsTab: React.FC<Props> = ({ taskforce, closeEditMode }) => {
  const [updateTaskforceUnit] = useUpdateTaskforceUnitMutation();
  ...
  const taskforceUnitsForm: EditUnitsSchemaType = useMemo(() => {
    return {
      ...
      logo: taskforce.unit.logo || '',
      subordinates: flattenSubordinates(taskforce.subordinates),
      location: taskforce.location || '',
      ownerId: taskforce.owner?.userId || '',
    };
  }, [taskforce]);

  // React Hook Form Methods
  const methods = useForm<EditUnitsSchemaType>({
    mode: 'all', // Validation Mode All = On Blur, On Change, On Submit
    resolver: zodResolver(editUnitsSchema), // Zod resolver for validations
    defaultValues: taskforceUnitsForm, // Default values
  });
  ...
  const handleSaveChanges = async (formValues: EditUnitsSchemaType) => {
    const formData = getUnitFormData(formValues);
    const errorMessage = 'Error updating your taskforce.';

    try {
      const response = await updateTaskforceUnit({ uic: taskforce.unit.uic, formData: formData });
