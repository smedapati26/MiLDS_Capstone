...
import { createPmxPalette, PmxPalette } from '@ai2c/pmx-mui/theme';
...
export const griffinPalette: PmxPalette = {
  dark: darkPalette,
  light: lightPalette,
};

export const griffinComponents: Components<Theme> = {
  MuiTablePagination: {
    styleOverrides: {
  ...
  MuiDataGrid: {
    styleOverrides: {
      root: ({ theme }: { theme: Theme }) => ({
        '--DataGrid-pinnedBackground': theme.palette.layout?.background16,
        '--DataGrid-containerBackground': theme.palette.layout?.background16,
        '.MuiDataGrid-columnHeaders': {
          backgroundColor: theme.palette.layout?.background16,
        },
        '.MuiDataGrid-row': {
          '&:nth-of-type(even)': {
  ...
  MuiTable: {
    styleOverrides: {
      root: ({ theme }: { theme: Theme }) => ({
        thead: {
          tr: {
            backgroundColor: `${theme.palette.layout?.background16} !important`,
          },
          th: {
...
export const griffinThemeOptions = {
  palette: griffinPalette,
  components: griffinComponents,
};
