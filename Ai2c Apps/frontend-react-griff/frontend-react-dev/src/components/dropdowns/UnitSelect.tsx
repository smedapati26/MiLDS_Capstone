...
export const UnitSelect: React.FC<UnitSelectProps> = ({
  ...
  const [selection, setSelection] = useState<FuzzySearchUnit | undefined>(
    value ? { ...value, searchValue: getFuzzySearchString(value) } : undefined,
  );
  ...
  useEffect(() => {
    const fuzzyUnits = units.map((unit) => ({
      ...unit,
      level: !unit.parentUic ? 0 : unit.level, // If no parent UIC set level to 0 or it will hide lower level uics
  ...
  const handleClearSection = (event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  // Unit selection
  const handleSelection = (event: React.MouseEvent<HTMLLIElement, MouseEvent>, unit: FuzzySearchUnit) => {
    const target = event.target as HTMLElement;
    if (target.tagName === 'DIV') {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const { searchValue, ...unitWithoutSearch } = unit;
      setSelection(unit);
      setUnitSearchAnchorEl(null);
      onChange(unitWithoutSearch);
    }
  };

  // clearing out or updating value
  useEffect(() => {
    if (!value || value?.uic === '') {
      setSelection(undefined);
    } else {
      setSelection({ ...value, searchValue: getFuzzySearchString(value) });
    }
  }, [value]);
    ...
    return filteredUnits
      ...
      .map((unit) => (
        <StyledTreeItem
          ...
          itemId={unit.uic}
          label={shortname ? unit.shortName : unit.displayName}
          onClick={(event) => handleSelection(event, unit)}
        >
  ...
  return (
    <>
      ...
      <Stack sx={{ width }}>
        {/* Select Label/Input */}
        <TextField
          id={`${id}-select-text-field`}
          data-testid={`${id}-select-text-field`}
          label={label}
          fullWidth={!width}
          size={size}
          sx={{
          ...
          value={selection ? (shortname ? selection.shortName : selection.displayName) : ''}
          onClick={!readOnly ? handleUnitSearch : () => {}}
          inputProps={{ readOnly, 'data-testid': `input-${id}-select-text-field` }}
          disabled={disabled}
          error={error}
          helperText={helperText}
          InputProps={{
            endAdornment: selection && !readOnly && !disabled && onClear && (
              <InputAdornment position="end">
                <IconButton
                  aria-label="unit-selection-clear-selection"
                  onClick={handleClearSection}
                  edge="end"
                  size="small"
                  data-testid={`${id}-unit-clear-button`}
                >
        ...
        >
          ...
          >
            {filteredUnits.map((unit) => {
              if (unit.level === unitLevel) {
                return (
                  <StyledTreeItem
                    id={`${id}-tree-item-${unit.uic}`}
                    data-testid={`${id}-tree-item-${unit.uic}`}
                    key={unit.uic}
                    itemId={unit.uic}
                    label={shortname ? unit.shortName : unit.displayName}
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
