...
from aircraft.models import Aircraft, AppliedModification, Modification, ModificationCategory
...
def create_applied_modification(request: HttpRequest, name: str):
    ...
    if modification_key == "category" and modification.type != ModificationTypes.CATEGORY:
        return HttpResponseBadRequest("Modification {} is not of type Category.".format(modification.name))
    elif modification_key == "category" and modification.type == ModificationTypes.CATEGORY:
        try:
            modification_value = ModificationCategory.objects.get(modification=modification, value=modification_value)
    ...
    elif modification_key == "status" and modification_value not in EquipmentStatuses:
        modification_value = EquipmentStatuses.UNK
    elif modification_key not in ["status", "category"] and not valid_modification_data_type(
