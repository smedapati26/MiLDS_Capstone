...
def get_1352(request: HttpRequest, uic: str, start_date: date = None, end_date: date = None):
    ...
    if requested_unit.uic.startswith("TF-"):
        task_force = TaskForce.objects.get(uic=requested_unit)
        task_force_ac = Aircraft.objects.filter(uic=requested_unit)
        data = DA_1352.objects.filter(
            serial_number__in=task_force_ac,
            reporting_month__range=(start_date, end_date),
            reporting_month__gte=task_force.start_date,
        ).values(*readiness_columns)
    else:
