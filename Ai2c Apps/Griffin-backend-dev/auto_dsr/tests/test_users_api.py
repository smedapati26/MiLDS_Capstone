...
from auto_dsr.api.users.routes import login_router, router
...
class UsersTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.create_user_expected_data = {
            ...
            "default_unit": {
                ...
                "parent_unit": self.unit.parent_uic,
    ...
    def test_user_api_whoami(self) -> None:
        ...
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertEqual(response.data["user_id"], "0000000000")

    def test_user_api_whoami_dne(self) -> None:
        """Test who-am-i api with a user that does not exist"""
        ...
        response = client.get("", headers={"Auth-User": "test.user"})
        self.assertEqual(response.status_code, HTTPStatus.OK)
