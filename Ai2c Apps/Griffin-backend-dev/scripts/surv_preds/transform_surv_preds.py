...
from aircraft.models import Aircraft, RawSurvivalPredictions, ShortLife, SurvivalPredictions
...
# Fetch all ShortLife objects in a single query with related objects
shortlife_qs = ShortLife.objects.select_related("aircraft").all()
shortlife_dict = {(sl.work_unit_code, sl.comp_serial_number, sl.part_number): sl for sl in shortlife_qs}

# Step 2: Iterate over the records:
for raw_prediction_record in tqdm(raw_predictions_qs):
    key = (
        raw_prediction_record["work_unit_code"],
        raw_prediction_record["serial_number"],
        raw_prediction_record["part_number"],
    )
    shortlife_record = shortlife_dict.get(key)
    if shortlife_record:
        aircraft = shortlife_record.aircraft
        nomenclature = shortlife_record.nomenclature
    else:
        continue
    ...
    try:
        ...
        prediction_record = SurvivalPredictions.objects.create(
            aircraft=aircraft,
            work_unit_code=raw_prediction_record["work_unit_code"],
