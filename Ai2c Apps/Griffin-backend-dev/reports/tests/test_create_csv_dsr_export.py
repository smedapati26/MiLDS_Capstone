...
class CreateCSVDSRExportTestCase(TestCase):
    ...
    def test_create_dsr_for_non_existent_unit(self):
        res = self.client.get("/dsr/csv/INVALID")
    ...
    def test_create_dsr(self):
        ...
        response = self.client.get(f"/dsr/csv/{self.units[0].uic}")
