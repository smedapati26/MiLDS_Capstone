...
def create_dsr_export_html(
    ...
    if body:
        ...
        custom_models = body.get("models", [])
        custom_models = [custom_models] if type(custom_models) == str else custom_models
        history_date = body.get("history_date", None)
        if history_date:
            history_date = datetime.strptime(history_date, "%Y-%m-%d")
