...
from reports.daily_status_report.utils import sort_inspection_columns, style_dsr_column
...
def dsr_table_col_widths(
    ...
    column_width_max_min = {
        ...
        "mods": {"min": (longest_mod / 8) * QUARTER_INCH, "max": HALF_INCH + 10},
    }
    column_widths = {
        column: max_min["min"]
    ...
    difference = LANDSCAPE_FULL_FRAME_WIDTH - (sum(column_widths.values()) - column_widths["remarks"])
    column_widths["remarks"] = max(column_widths["remarks"], difference)
...
def dsr_df_to_table_list(df: pd.DataFrame, inspections_column_count: int, mods_column_count: int) -> list[list]:
    ...
    unit = Unit.objects.get(uic__in=df["current_unit"])
    unit_info = [f"Current Unit: {unit.uic} - {unit.short_name}"]
    return [column_names, unit_info, *aircraft_data]
...
def dsr_details_table_style(
    ...
    # Sub Header
    style.add("FONTNAME", (0, 1), (-1, 1), "Roboto")
    style.add("BACKGROUND", (0, 1), (-1, 1), "#276db2")
    style.add("TEXTCOLOR", (0, 1), (-1, 1), "#FFFFFF")
    style.add("SPAN", (0, 1), (-1, 1))
    style.add("ALIGNMENT", (0, 1), (-1, 1), "LEFT")
    # Resize Location Column Font to Fit
    style.add("FONTSIZE", (2, 2), (2, -1), location_font_size)
    # Model Column
    current_model = None
    group_start = 2
    style.add("SPAN", (0, 2), (0, -1))
    for i, model in enumerate(df.model.to_list()):
        # Account for sub header row
        i += 1
    ...
    style.add("FONTNAME", (0, 2), (0, -1), "Roboto-Bold")
    # Row Backgrounds
    for i in range(1, df.shape[0], 2):
        style.add("BACKGROUND", (1, i + 1), (-1, i + 1), "#F2F2F2")

    df = df.sort_values(by=["serial"])
    conditional_formatting_styles = []
    # Conditionally Format Status
    conditional_formatting_styles.extend(style_dsr_column(df["status"], 4, "status", row_start=2))
    # Conditionally Format PMI
    conditional_formatting_styles.extend(style_dsr_column(df["hours_to_phase"], 9, "phase", df["model"], row_start=2))
    ...
    for i, insp_column in enumerate(insp_dsr_columns):
        conditional_formatting_styles.extend(
            style_dsr_column(df[insp_column], columns_before_inspections + i, "inspection", row_start=2)
        )
    ...
    for i, mod_column in enumerate(mods_dsr_columns):
        conditional_formatting_styles.extend(
            style_dsr_column(df[mod_column], columns_before_mods + i, "status", row_start=2)
        )
