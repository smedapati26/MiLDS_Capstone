...
def draw_unit_phase_flow_graph(
    canvas: Canvas, model_barcharts: list[VerticalBarChart], unique_models: list[str], debug: bool = False
) -> int:
    """
    ...
    try:
        for i in range(0, len(model_barcharts)):
            ...
            horizontal = Line(
                50 * size_multiplier, 112.5 * size_multiplier, 350 * size_multiplier, 112.5 * size_multiplier
            )
