...
from reports.daily_status_report.components.draw import draw_footer, draw_header, draw_unit_nrtl_table
from reports.daily_status_report.constants import DSR_DETAILS_FRAME_HEIGHT, LANDSCAPE_FULL_FRAME_WIDTH
...
class UnitNRTLPage(Page):
    ...
    def assemble_representation(self):
        ...
        if not self.aircraft_data.empty:
            ...
            self.nrtl_data = self.nrtl_data[["serial", "model", "current_unit", "status", "rtl", "remarks"]]
            ...
            self.nrtl_table_2 = None
            _, table_height = self.nrtl_table.wrap(LANDSCAPE_FULL_FRAME_WIDTH, DSR_DETAILS_FRAME_HEIGHT)
            # Check if the table fits within the frame
            if table_height >= DSR_DETAILS_FRAME_HEIGHT:
                midpoint = len(self.nrtl_data) // 2
                nrtl_data_first_half = self.nrtl_data.iloc[:midpoint]
                nrtl_data_second_half = self.nrtl_data.iloc[midpoint:]
                self.nrtl_table = assemble_nrtl_table(nrtl_data_first_half)
                self.nrtl_table_2 = assemble_nrtl_table(nrtl_data_second_half)

        if not self.uas_data.empty:
            self.uas_nrtl_data = self.uas_data[self.uas_data["rtl"] == "NRTL"]
            self.uas_nrtl_data = self.uas_nrtl_data[
                ["serial_number", "model", "current_unit", "status", "rtl", "remarks"]
            ]
    ...
    def draw_representation(
        ...
        if not self.nrtl_data.empty:
            ...
            # Finish page
            canvas.showPage()
            if self.nrtl_table_2:
                # Draw header and footer
                draw_header(self.unit, canvas, logos, subheader="NRTL Summary - Rotary Wing Cont.")
                draw_footer(self.unit, canvas, next_page_number, positions)
                next_page_number += 1
                # Draw report content
                draw_unit_nrtl_table(canvas, self.nrtl_table_2)
                # Finish page
                canvas.showPage()
