...
from reports.daily_status_report.components.assemble import assemble_rtl_summary_table, assemble_slant_table
...
class UnitSummaryPage(Page):
    ...
    def fetch_data(self):
        ...
        self.slant_data = unit_slant_data_for(self.unit, self.config.include_uas, self.config.custom_models)
        self.rtl_data = unit_summary_rtl_data_for(self.unit, self.config.include_uas, self.config.custom_models)
    ...
    def assemble_representation(self):
        ...
        self.rtl_tables = assemble_rtl_summary_table(self.rtl_data)
    ...
    def draw_representation(
        ...
        # Draw report content
        draw_unit_rtl_summary(canvas, self.rtl_tables, self.config.debug)
