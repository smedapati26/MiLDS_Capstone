...
class UnitForscomReportPage(Page):
    ...
    def fetch_data(self):
        ...
        self.aircraft_data = unit_rw_data_for(self.unit, self.config.custom_models)
    ...
    def draw_representation(
        ...
        if not self.aircraft_data.empty:
            ...
            # Draw report content
            draw_report(
                self.unit, canvas, logos, self.aircraft_data, self.uas_data, self.aircraft_fhp_data, self.unit_positions
            )
            # Finish page
            canvas.showPage()
