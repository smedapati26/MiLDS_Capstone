...
from agse.models import AGSE, AgseEdits, UnitAGSE
...
def create_test_agse_in_all(units: list[Unit], num_of_agse: int = 1, location: Location | None = None) -> list[AGSE]:
    """
    ...
    for unit in units:
        if unit.echelon == "CO":
            ...
            for i in range(num_of_agse):
                ...
                new_agse = create_single_test_agse(
                    current_unit=unit, equipment_number=equipment_number, location=location
                )
...
def create_single_test_agse(
    ...
    @param equipment_number: (str) The primary key value for the new AGSE
    ...
    @param nomenclature: (str) The equipment nomenclature value for the new AGSE
    @param display_name: (str) The equipment display name value for the new AGSE
    @param condition: (AgseStatus) The maintenance status value for the new AGSE
    ...
    @param location: (Location | None) The location value for the new AGSE; can be None
