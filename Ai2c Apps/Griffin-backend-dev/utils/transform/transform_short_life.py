...
def transform_short_life():
    ...
    new_records = []
    update_records = []
    for raw_shortlife_record in raw_shortlife_qs:
        ...
        try:
            ...
            update_records.append(shortlife_record)
        except ShortLife.DoesNotExist:
            shortlife_record = ShortLife(
                aircraft=aircraft,
                work_unit_code=raw_shortlife_record["work_unit_code"],
                nomenclature=(
                    raw_shortlife_record["nomenclature"] if raw_shortlife_record["nomenclature"] else "UNKNOWN"
            ...
            new_records.append(shortlife_record)

    ShortLife.objects.bulk_create(new_records)
    ShortLife.objects.bulk_update(
        update_records,
        [
            "aircraft",
            "work_unit_code",
    ...
    return f"Transformation completed successfully! Added {len(new_records)} new ShortLife records and updated {len(update_records)} records."
