...
from utils.tests import create_logical_clock, create_test_soldier, create_testing_unit, create_user_role_in_all
...
class UnitApiTest(TestCase):
    ...
    def setUpClass(test_class):
        super().setUpClass()
        test_class.patcher = patch("units.api.routes.get_user_id")
        test_class.get_user_id = test_class.patcher.start()
        test_class.addClassCleanup(test_class.patcher.stop)

    def setUp(self):
        self.headers = {"Auth-User": "CN=KEROUAC.JACK.A.0000000000,OU=USA,OU=PKI"}
        ...
        self.legacy_unit = create_testing_unit()
        self.user = create_test_soldier(unit=self.legacy_unit, user_id="0000000000", is_admin=True)
        self.d_co_data = {
    ...
    def test_list_units_filter_by_role(self):
        """Test filtering units by user role"""
        UserRole.objects.create(user_id=self.user, unit=self.co_a, access_level=UserRoleAccessLevel.MANAGER)
        UserRole.objects.create(user_id=self.user, unit=self.co_b, access_level=UserRoleAccessLevel.ADMIN)

        res = self.client.get(f"?role={UserRoleAccessLevel.MANAGER}")
        self.assertEqual(res.status_code, HTTPStatus.OK)
        units = res.json()
        self.assertEqual(len(units), 1)
        self.assertEqual(units[0]["uic"], self.co_a.uic)

    def test_list_units_filter_by_role_multiple(self):
        """Test that user with multiple roles can filter by specific role"""
        UserRole.objects.create(user_id=self.user, unit=self.co_a, access_level=UserRoleAccessLevel.MANAGER)
        UserRole.objects.create(user_id=self.user, unit=self.co_b, access_level=UserRoleAccessLevel.MANAGER)
        UserRole.objects.create(user_id=self.user, unit=self.co_c, access_level=UserRoleAccessLevel.ADMIN)

        res = self.client.get(f"?role={UserRoleAccessLevel.MANAGER}")
        self.assertEqual(res.status_code, HTTPStatus.OK)
        units = res.json()
        self.assertEqual(len(units), 2)
        unit_uics = [u["uic"] for u in units]
        self.assertIn(self.co_a.uic, unit_uics)
        self.assertIn(self.co_b.uic, unit_uics)

    def test_list_units_filter_by_role_no_results(self):
        """Test filtering by role when user has no units with that role"""
        UserRole.objects.create(user_id=self.user, unit=self.co_a, access_level=UserRoleAccessLevel.MANAGER)

        res = self.client.get(f"?role={UserRoleAccessLevel.ADMIN}")
        self.assertEqual(res.status_code, HTTPStatus.OK)
        self.assertEqual(len(res.json()), 0)

    def test_list_units_filter_by_role_combined_with_other_filters(self):
        """Test role filter combined with other filters"""
        self.set_unit_lists()

        UserRole.objects.create(user_id=self.user, unit=self.co_a, access_level=UserRoleAccessLevel.MANAGER)
        UserRole.objects.create(user_id=self.user, unit=self.co_c, access_level=UserRoleAccessLevel.MANAGER)

        res = self.client.get(f"?role={UserRoleAccessLevel.MANAGER}&top_level_uic={self.bn.uic}")
        self.assertEqual(res.status_code, HTTPStatus.OK)
        units = res.json()
        self.assertEqual(len(units), 2)
