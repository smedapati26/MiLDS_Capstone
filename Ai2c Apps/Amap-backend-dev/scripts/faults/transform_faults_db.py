...
# def transform_faults_db(filter_date: str | None):
...
#     fault_query = "SELECT * FROM raw_amap_faults WHERE fault_sync_timestamp >= '{}'".format(filter_date)
#     fault_action_query = "SELECT * FROM raw_amap_fault_actions WHERE fault_action_sync_timestamp >= '{}'".format(
#         filter_date
#     )
...
#     # Get raw faults from time window specified, create new faults
#     with connection.cursor() as cursor:
#         cursor.execute(fault_query)
#         columns = [col[0] for col in cursor.description]
#         fault_records = [dict(zip(columns, row)) for row in cursor.fetchall()]
#         print(len(fault_records))

#         for record in tqdm(fault_records):
#             try:
#                 Fault.objects.get(id=record["id"])
...
#     # Get raw fault actions from time window specified, create new fault actions
#     with connection.cursor() as cursor:
#         cursor.execute(fault_action_query)
#         columns = [col[0] for col in cursor.description]
#         fault_action_records = [dict(zip(columns, row)) for row in cursor.fetchall()]
#         print(len(fault_action_records))

#         for record in tqdm(fault_action_records):
#             try:
#                 soldier = Soldier.objects.get(user_id=record["personnel_dodid"])
#                 try:
...
#     end_time = time.time()
#     print("API Call took {} seconds".format(str(end_time - start_time)))

#     return "Successfully Transformed New Faults from Vantage."
...
# transform_faults_db("2025-03-13")
