...
7. Run Django Server
   ```
   # Run django server
   py manage.py runserver --settings=amap.settings.dev.cdso
   ```
...
py manage.py makemigrations --settings=amap.settings.dev.cdso
# Apply those changes using the migrate command
py manage.py migrate --settings=amap.settings.dev.cdso


py manage.py createsuperuser --settings=amap.settings.dev.cdso
